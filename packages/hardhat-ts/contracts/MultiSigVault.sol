...
contract MultiSigVault {
  ...
  event SubmitTransaction(address indexed owner, uint256 indexed txIndex, address indexed to, uint256 value, bytes data, uint256 timestamp);
  event ConfirmTransaction(address indexed owner, uint256 indexed txIndex);
  event RevokeConfirmation(address indexed owner, uint256 indexed txIndex);
  event ExecuteTransaction(address indexed owner, uint256 indexed txIndex, uint256 timestamp);
  ...
  constructor(address[] memory _owners, uint256 _confirmationsRequired) public {
  ...
  function executeTransaction(uint256 _txIndex) public onlyOwner txExists(_txIndex) notExecuted(_txIndex) {
    ...
    require(transaction.numConfirmations >= confirmationsRequired, 'cannot execute tx');

    require(address(this).balance >= transaction.value, 'insufficient funds in wallet');
